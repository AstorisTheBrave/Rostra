...
The hybrid path is wired and type-checked but, like the music or Lavalink path, it needs a real
multi-process or multi-host deployment to smoke-test. Validate it in staging before pointing production at
`SHARDING_MODE=hybrid`.